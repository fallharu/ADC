...
def fetch_directions(conn, run_id):
    """Fetch all detection directions for a run to map (frame, group) -> direction."""
    try:
        query = "SELECT frame_num, group_id, travel_direction FROM Detection WHERE run_id = ?"
        df = pd.read_sql_query(query, conn, params=(run_id,))
        # Create dictionary: (frame, group) -> direction
...
def fetch_lane_lines_from_db(conn, run_id):
    """Fallback if calibration json missing."""
...
def get_run_id_by_filename(conn, filename):
    """Find the latest run_id for a given video filename."""
    # Remove path if present in CSV, though usually it's just filename
    filename = os.path.basename(filename)
    
    # ProcessLog (runs) links to Video via video_id.
    query = """
        SELECT p.run_id 
        FROM ProcessLog p
...
def calculate_distance(row, direction_map, calibration_data, lane_polygon, scale_m_per_px):
    ...
    # Calculate simple BBox horizontal distance (Clearance)
    # If Bike is on Left (smaller X) and Car on Right (larger X): Clearance = Car.x1 - Bike.x2
    clearance_px = 0
    if on_x2 < ot_x1: 
    ...
    def get_line_x(line_points, y):
        ...
        lx = None
        # Use the bottom-center of the overtaker's bbox as the reference Y for line intersection
        y_center = ot_y2 # Assuming bottom of bbox is the most relevant point for ground plane
    ...
    if target_left:
        lx_at_y = get_line_x(target_left, ot_y2) # Use bottom of bbox
        if lx_at_y is not None:
            dist_px = ot_x1 - lx_at_y # Left Edge - Line X (Positive if inside lane/right of line)
            line_dist_m = dist_px * scale
            
    # Calculate Right Distance
    if target_right:
        rx_at_y = get_line_x(target_right, ot_y2) # Use bottom of bbox
        if rx_at_y is not None:
            # Right Edge vs Line X
            # If Line is at 1300 and Car Right is 1200: Dist = 1300 - 1200 = 100 (Positive inside)
...
def main():
    ...
    for video_name, group_df in grouped:
        ...
        # 2. Fetch Directions (Optional context)
        ...
        calib_data = {}
        try:
            # We already have video_name (basename presumably)
        ...
        for idx, row in group_df.iterrows():
            ...
            # Update Left White Line Metrics
            if l_m is not None:
            ...
            # Update Right White Line Metrics
            if r_m is not None:
            ...
            # Update Generic "White Line Distance" (Overtake side, usually left)
