"""
使っていないテストプログラムと一時ファイルの整理

このスクリプトは削除候補のファイルをカテゴリ別に整理し、
ユーザーの確認後に削除を実行します。
"""
...
CLEANUP_CANDIDATES = {
    ...
    "scripts内検証・分析（保留）": [
        ...
        # "scripts/extract_db_stats.py",
        # "scripts/extract_db_summary.py",
    ],
}
...
def print_summary():
    """削除候補ファイルのサマリーを表示"""
    print("=" * 80)
    print("使っていないテストプログラムと一時ファイルの整理")
    print("=" * 80)
    print()
    ...
    for category, files in CLEANUP_CANDIDATES.items():
        ...
        if category_size > 0:
            print(f"  小計: {category_size / 1024 / 1024:.2f} MB")
    
    print("\n" + "=" * 80)
    print(f"合計: {total_files}個のファイル, {total_size / 1024 / 1024:.2f} MB")
    print("=" * 80)


def create_cleanup_script():
    """削除スクリプトを生成"""
    script_path = PROJECT_ROOT / "cleanup_files.bat"
    
    with open(script_path, 'w', encoding='utf-8') as f:
        f.write("@echo off\n")
        f.write("echo テストファイルとデバッグファイルを削除します...\n")
        f.write("echo.\n")
        f.write("pause\n")
        f.write("echo.\n\n")
        
        for category, files in CLEANUP_CANDIDATES.items():
            f.write(f"REM {category}\n")
            for file_path in files:
                full_path = PROJECT_ROOT / file_path
                if full_path.exists():
                    f.write(f'del /f "{file_path}"\n')
            f.write("\n")
        
        f.write("echo.\n")
        f.write("echo 削除が完了しました。\n")
        f.write("pause\n")
    
    print(f"\n削除スクリプトを生成しました: {script_path}")
    print("実行するには、cleanup_files.bat をダブルクリックしてください。")
...
if __name__ == "__main__":
    ...
    response = input("削除スクリプトを生成しますか? (y/N): ")
    if response.lower() == 'y':
